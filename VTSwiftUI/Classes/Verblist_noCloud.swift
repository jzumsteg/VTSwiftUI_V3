...
// July 2017: change: store verblists with the uuid as the file name, rather than the verb list name
import Foundation
import CloudKit

let keyData: String = "keyData"

class Verblist: Identifiable, Hashable {
    static func == (lhs: Verblist, rhs: Verblist) -> Bool {
...
//    }
    ...
    var fullPath: String { // includes full path and extension
        get {
            return FileUtilities.verbListDirectoryPath().stringByAppendingPathComponent(path: name).stringByAppendingPathExtension(ext: "verblist")!
        }
    }
    
    var fullURL: URL {
            get {
                return FileUtilities.verbListURL(name)
            }
        }// full path and extension
    
    var isDirty: Bool {
        willSet {
            Log.print("Changing isDirty from \(isDirty) to \(newValue)")
        }
    }
    
    var isNew: Bool {
        willSet {
            Log.print("Changing isNew to \(newValue)")
        }
    }
    ...
    func addVerb(verb: String) {
        let index = infinitives.firstIndex(of: verb)
    ...
    func save() {
        ...
        isNew = false
//        if isNew {
// don't know how or why we create a new record on the cloud.
//            createNewRecordInCloudKit(dt: modDateTime)
            //. until we implement Cloudkit, save it locally
//            saveLocally(dt: modDateTime)
//        }
//        else {
//            saveLocally(dt: modDateTime)
//        }
    ...
    func saveLocally(dt: Date) {
        ...
        let fullPath = FileUtilities.verbListPath(file: name).stringByAppendingPathExtension(ext: "verblist")
...
//        let fpath = FileUtilities.verbListPath(file: name)
    ...
    func deleteVerbList(verblistname: String) {
    ...
    func retrieveLocally() {
        ...
        if let fpath = FileUtilities.verbListPath(file: name.stringByAppendingPathExtension(ext: "verblist")!) {
            guard let dic = NSMutableDictionary(contentsOfFile: fpath) else {
                Params.shared.currentSelectedVerblist = nil
            ...
            if let l_uuid = dic.object(forKey: "uuid") {
                uuid = l_uuid as? String
            }
        }
        isDirty = false
        isNew = false
    ...
    func retrieveLocally(url: URL) {
        infinitives.removeAll()
        
        let dic = NSMutableDictionary(contentsOfFile: url.path)!
...
//        deleteFromCloudKit()
    ...
    func printYourself() {
        ...
        Log.print("   uuid: \(String(describing: uuid))")
...
//        Log.print("   fullURL: \(fullURL)")
    ...
    func copy(with zone: NSZone? = nil) -> Verblist {
        let copy = Verblist(verblistname: name)
        copy.name = name
